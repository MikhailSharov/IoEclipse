...
Pro Io_Eclipses_optical, Part=Part, Date=Date
  ...
  if date ne !null then begin
    Case 1 of
      ...
      date eq 'UT200908': begin      ; **IFFY pointing** 19, 20, 23 are good, rest iffy at best
        ...
        reduced_dir           = 'D:\DATA\Apache Point\Echelle\Io Eclipses\'+date+'\Reduced\'
        calibration_dir       = 'D:\DATA\Apache Point\Echelle\Io Eclipses\'+date+'\Reduced\'  ; Directory with Stellar Spectra for Telluric Corrections
        Eclipse_files         = ['Io_eclipsed.0019', 'Io_eclipsed.0020', 'Io_eclipsed.0023', 'Io_eclipsed.0025', 'Io_eclipsed.0026']  ;  until frame 22 Frame 24 is clearly in penumbra despite the name
  ...
  if part eq 0 then begin
    ...
    ; Where telluric absorption corrections are needed, carefully identify wavelength indicies where we can isolate the absorption
      ...
      ;if (Io_Airglow[i] eq  Na3) then stop
    endfor
...
;        endif
  ...
  if Part eq 1.00 then begin ; Make waterfall plots
    ...
    Case date of
      'UT180320': begin
        Fit_Me_To_The_Scatter    = ['D:\DATA\Apache Point\Echelle\Io Eclipses\UT180320\Reduced\R_per_A_Jovian_Scatter.0001.ec.fits', $
                                    'D:\DATA\Apache Point\Echelle\Io Eclipses\UT180320\Reduced\R_per_A_Jovian_Scatter.0002.ec.fits', $
...
;            endcase
  ...
  ;aim for part 1 and 12 to do everything APO!
  ...
  if part eq 100.0 then begin
    ...
    MWRFITS, rotate(transpose(FLAT),7), directory+'Processed\FLAT_BS.fits', header, /create, /silent ;super flat, bias subtracted but ***NOT NORMALIZED***
    ...
    MWRFITS, rotate(transpose(ThAr),7), directory+'Processed\ThAr.fits', header, /create, /silent
    ...
    for i = 0, n_elements(Jupiter_frames)-1 do begin
      ...
      MWRFITS, write_file, directory+'Processed\' + new_filename + '.Cleaned.fits', header, /create, /silent
    endfor
    ...
    MWRFITS, rotate(transpose(Ganymede),7), directory+'Processed\j2970302.Trace.fits', header, /create, /silent
    ...
    Io_Airglow_params = create_struct( 'torus_lat', torus_lat_array, 'ET', ET_array, 'ExpTime', exptime_array, Io_Airglow_params )
    print, 'XD Angle:', Sxpar(header, 'XDANGL'), ' Echelle Angle:', Sxpar(header, 'ECHANGL')
    save, Io_Airglow_params, filename = Directory + 'Processed\Io_Airglow_params.sav'
  ...
  if part eq 101. then begin ; flatten, extract and get the wavelength solutions
    ...
    for h = 0, N_elements(orders) - 1 do begin
      order = orders[h]
      if not ((order.name eq 'order_43')) then continue
      ;if h le 15 then continue

      ; Use Ganymede to find the trace in within the orders of interest
      G    = mrdfits(Directory + 'Processed\j2970302.Trace.fits', 0, G_header) ; do not use multiple Ganymede frames here, it's important it's fixed.
      ThAr = mrdfits(Directory + 'Processed\ThAr.fits', 0, ThAr_header)
      flat = mrdfits(directory+'Processed\FLAT_BS.fits', 0, Flat_header)       ; not yet normalized to unity
      ...
      for frame = 0, n_elements(Frames)-1 do begin
        ...
        Io   = mrdfits(Directory + 'Processed\' + Frames[frame], 0, header)

        ; -------------------------------------- Wavelength Solution in the Orders of Interest ------------------------------------------------------------------
        ;              Take_a_long_hard_look   = mrdfits(Directory + 'Processed\' + Frames[-2], 0, header)
        ...
        ; Manaul override, when this technique fails
        ...
        MWRFITS, aperture, directory + 'Processed\Cosmic Rays\CR_' + Frames[frame], header, /create
        ...
        endcase
        la_cosmic, directory + 'Processed\Cosmic Rays\CR_' + Frames[frame], outsuff = "CR", sigclip = 4.5, statsec = statsec
        cube[*,*,frame] = mrdfits(Directory + 'Processed\Cosmic Rays\CR_j2970'+strcompress(Io_frames[frame], /rem)+'.CleanedCR.fits', 0, junk_header)
      ...
      for frame = 0, n_elements(Frames)-1 do begin
        ...
        Jupiter  = mrdfits(Directory + 'Processed\' + Frames[frame], 0, header)
        subframe = G[0:4000, order.low_bound:order.hi_bound]
        s = size(subframe, /dim)
        trace = fltarr(s[0])
        for i = 0, s[0] - 1 do begin
      ...
      save, cube, Jupiter_cube, WL, filename = Directory + 'Processed\' + order.name + '.sav'
  ...
  if Part eq 102. then begin ;
    ...
    order           = order_56
    restore, Directory + 'Processed\' + order.name + '.sav'
    dummy           = Cube
    ...
    for f = 0, N_elements(Io_frames )-1 do begin
      ...
      ind        = sort(fit)
      sky_BG_ind = ind[0:3]   ; largest extraction regions over Io seem to show the best results, but we still need tome sky to sample at both ends
      Io_ind     = ind[4:*]
      ...
      ; Manual override, we want to always have some of both sides of the sLIt
    ...
    save, EXTRACT_ROWS, filename = Directory + 'Processed\EXTRACT_ROWS.sav'
  ...
  if Part eq 103. then begin ; Make waterfall plots in each order
    restore, Directory + 'Processed\Io_Airglow_params.sav'
    ...
    Jupiter_center_header = headfits(directory+'Processed\j2970'+strcompress(Jupiter_frames[0], /rem)+'.Cleaned.fits')
    ...
    restore, Directory + 'Processed\EXTRACT_ROWS.sav'
    ...
    for h = 0, N_elements(orders)-1 do begin
      ...
      cgPS_Open, filename = Directory + 'Processed\'+order.name+'_scatterfit.eps', /ENCAPSULATED, xsize = 7.5, ysize = 5
      !P.font = 1
      device, SET_FONT = 'Helvetica Bold', /TT_FONT
      !p.charsize = 1.5
      ...
      for i = 0, N_elements(Io_frames )-1 do begin
        ...
        spec                      = reform(Io_extract[*,i])
        scatter                   = reform(Sky_extract[*,i])
      ...
      cgps_close
      ;if order.name eq 'order_43' then stop
    endfor
