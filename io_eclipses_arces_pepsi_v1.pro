...
Pro Io_Eclipses_ARCES_PEPSI_V1, Part=Part, Date=Date
...
;                                      ((wl gt 6287.0) and (wl lt 6291.)) or ((wl gt 6292.0) and (wl lt 6293.0)), /NULL)
    ...
    cgLoadCT, 33, NColors=8, /reverse
    pos = cgLayout([1,2], OXMargin=[18,4], OYMargin=[13,7], YGap=0)
    ...
    cgPS_Open, filename = Reduced_Dir+'O6300_scatterfit_V1.eps', /ENCAPSULATED, xsize = 10, ysize = 8.
      ...
      for i = 0, n_elements(Eclipse_files)-1 do begin
        ...
        exclude_WLs            = where( ((wl gt O2_Io_frame-0.4) and (wl lt O2_Io_frame+0.4)) or ((wl gt O2-0.4) and (wl lt O2+0.4)), /NULL)
        scatter_fitting_ind    = cgSetDifference(include_WLs, exclude_WLs)
   
        P0 = [1.e-3, 0., 4., 5.]    
        p = mpfitfun('Match_Jup_Combo', Jup_Cal_Tell_Corr[scatter_fitting_ind], Spec[scatter_fitting_ind], spec_err[scatter_fitting_ind], P0, /NaN, status = Scatter_fit_status, parinfo = parinfo, /verbose)
        scatter_fit = interpolate(gauss_smooth(P[0]*Jup_Cal_Tell_Corr + P[1], P[2], /EDGE_TRUNCATE), findgen(n_elements(P[0]*Jup_Cal_Tell_Corr + P[1])) - P[3])
        ...
        residual_err = Spec_err  ;real replacement for hacked_errors, not sure if anything from scatter_fit should be subtracted. HACK HAck!
      ...
      AL_legend, ['Raw Io Data','Jupiter Scatter Fit'], Psym = [0,0], linestyle = [0,1], charsize = 1.5, /right
