...
dir             = 'D:\DATA\Apache Point\Echelle\Io Eclipses\UT180320\
reduced_dir     = 'D:\DATA\Apache Point\Echelle\Io Eclipses\UT180320\Reduced\'
...
;----------------------Compare 2 methods to get a decent telluric absorption spectrum---------------------------
  ...
  ; Method 2, A0V star with model spectral reference
    ...
    Model_A0V_Flux = float(shift(Model_A0V_Flux, -100)) ; HACK HACK HACK HACK 
    ...
    cgplot, WL, Model_A0V_norm, color = 'blue', xr = [5570,9300], yr = [-2,2]
...
;----------------------get the absolute spectral reflectivity of Jupiter---------------------------
  ...
  ; Woodman only gives I/F to get the output light from Jupiter, multiply this by a solar spectrum
    ...
    Jupiter_center_header = headfits(reduced_dir+'sumfitcontJupiter_Center_Spectrum.0001.ec.fits')
  ...
  ; get the counts per pixel on Jupiter's disk
    cont_fit    = MRDFITS(reduced_dir+'sumfitcontJupiter_Center_Spectrum.0001.ec.fits', 0, header, /fscale, /silent, /unsigned )
    jup_center_forfit  = MRDFITS(reduced_dir+'sumJupiter_Center_Spectrum.0001.ec.fits', 0, header, /fscale, /silent, /unsigned ) ;sum of jupitercenter, used for poly fit
    jup_center = MRDFITS(reduced_dir+'fullspecJupiter_Center_Spectrum.0001.ec.fits', 0, header, /fscale, /silent, /unsigned )    ;fullspec, normalized to one
    WL          = sxpar(header, 'CRVAL1')+findgen(N_elements(jup_center))*sxpar(header, 'Cdelt1')
    jup_center_err =  MRDFITS(reduced_dir+'sig_fullspecJupiter_Center_Spectrum.0001.ec.fits', 0, header, /fscale, /silent, /unsigned ) ;errors, normalized to one
    jup_center= jup_center* poly(WL, ROBUST_POLY_FIT(WL, jup_center_forfit, 10))                                                       ;fullspec unormalzied using poly fit
    jup_center_err= jup_center_err* poly(WL, ROBUST_POLY_FIT(WL, jup_center_forfit, 10))                                               ;fullspec errors unormalzied using poly fit
    raw_header  = headfits(dir+'Jupiter_Center_Spectrum.0001.fits')
  ...
  ; Correct tellurics and write the spectra into R/A units...   
    for i = 0, n_elements(Eclipse_files)-1 do begin 
      ...
      spec_err = spec_err*poly(WL, ROBUST_POLY_FIT(WL, spec_forfit, 10))
  ...
  ; Correct Jovian Scattered Light
    ...
    include_WLs            = where( ((wl gt 6290.) and (wl lt 6310.)), /NULL)
    exclude_WLs            = where( ((wl gt 6299.7) and (wl lt O2 + 0.1)), /NULL)
    scatter_fitting_ind    = cgSetDifference(include_WLs, exclude_WLs)
    LSF_fitting_ind        =  where( ((wl gt 6299.7) and (wl lt 6300.1)), /NULL)
    ...
    cgplot, WL, spec, xr = [6298,6302], yr = [8.e3, 9.e4], ytitle = 'R/A', /nodata
    ...
    for i = 0, n_elements(Eclipse_files)-1 do begin
      ...
      LSF_Fit = mpfitpeak(WL[LSF_fitting_ind], residual[LSF_fitting_ind], a, PERROR = err_a, /POSITIVE, PARINFO = Gaussian_parinfo, $
        ...
        Brightness_array[i]         = $   ;  total(residual[LSF_fitting_ind])
                                      A[0]*0.09*SQRT(2*!DPI)                                                      ; HACK HACK HACK HACK fix the linewidth used in the fitting
                                          ;  A[0]*A[2]*SQRT(2*!DPI)
...
stop
    ...
    pos =  [.17,.2,.98,.9]
    cgPS_Open, filename = Reduced_Dir+'O6300_lightcurve_V2.eps', /ENCAPSULATED, xsize = 10., ysize = 6.5
      !P.font=1
      ...
      cgplot, T_Shadow_array, Brightness_array, psym = 15, Ytitle = 'Rayleighs (5.1 sq arcsec aperture)', xtitle = 'Minutes After Ingress', title = 'Io''s Oxygen 6300'+cgsymbol('Angstrom') + ' Response to Eclipse',  $
              ERR_YLOW = ERR_Brightness_array, ERR_YHigh = ERR_Brightness_array, ERR_XLOW = EXPTime_array/2., ERR_XHigh = EXPTime_array/2., yr = [0, 1700], xr = [0,38], /nodata, pos = pos
      ...
      cgplot, T_Shadow_array, Brightness_array, psym = 16, /noerase, symsize = 2, yr = [0, 1700], xr = [0,38], pos = pos, $
        ERR_YLOW = ERR_Brightness_array, ERR_YHigh = ERR_Brightness_array, ERR_XLOW = EXPTime_array/2., ERR_XHigh = EXPTime_array/2.
      SAVE , FILENAME = "180320O6300.sav", T_Shadow_array, Brightness_array, ERR_Brightness_array
    AL_legend, ['O6300 in Ingress 180320'], Psym = 16, /right, charsize = 1.5
    cgps_close
    cgLoadCT, 33, NColors=8, /reverse
...
stop
    ...
    spec = MRDFITS(reduced_dir+'fullspecIo_eclipsed.0001.ec.fits', 0, header, /fscale, /silent, /unsigned )
    ...
    ;Smooth_array  = [0,  15,  14,  14,  14,  14,  14]  ; Good for 5893
    ;Smooth_array  = [0,  15,  14,  14,  14,  12,  12]  ; Good for 5893
  ...
  cgPS_Open, filename = Reduced_Dir+'Na_Reduction.eps', /ENCAPSULATED, xsize = 10., ysize = 6.5
    ...
    jup = MRDFITS(reduced_dir+'fullspecJupiter_Center_Spectrum.0001.ec.fits', 0, header, /silent, /fscale, /unsigned )
...
;hmmm... below looks much worse than above... why???!!!
    ...
    include_WLs            = where( ((wl gt 5888.) and (wl lt 5898.)), /NULL)
    exclude_WLs            = where( ((wl gt 5889.45) and (wl lt 5889.74)), /NULL)
    exclude_WLs2            = where( ((wl gt 5895.4) and (wl lt 5895.72)), /NULL)
    scatter_fitting_ind    = cgSetDifference(include_WLs, exclude_WLs)
    scatter_fitting_ind    = cgSetDifference(scatter_fitting_ind, exclude_WLs2)
    LSF_fitting_ind        =  where( ((wl gt 5889.) and (wl lt 5890.)), /NULL)
    ...
    for i = 1, n_elements(Eclipse_files)-1 do begin
      ...
      cgplot, WL, scatter_fit + i*4.e3, /OVERPLOT, COLOR = timeColors[i], linestyle =3 ; offset each plot
      cgplot, [5889.45, 5889.45] , [0, 1e6], /overplot ; plotting the edge cases of the scatter fitting indicies
      cgplot, [5889.74, 5889.74] , [0, 1e6], /overplot
      cgplot, [5895.45, 5895.45] , [0, 1e6], /overplot
      cgplot, [5895.72, 5895.72] , [0, 1e6], /overplot
      residual = y - scatter_fit
      residual_err = err_y
...
stop
    ...
    cgplot, T_Shadow_array, Brightness_array, psym = 16, /noerase, symsize = 2, yr = [0, 1700], xr = [0,38], pos = pos, $
      ERR_YLOW = ERR_Brightness_array, ERR_YHigh = ERR_Brightness_array, ERR_XLOW = EXPTime_array/2., ERR_XHigh = EXPTime_array/2.
    AL_legend, ['NA5888 in Egress 180320'], Psym = 4, /right, charsize = 1.5
    cgps_close
...
include_WLs            = where( ((wl gt 7769.) and (wl lt 7778.)), /NULL)
LSF_fitting_ind        =  where( ((wl gt 5889.) and (wl lt 5890.)), /NULL)
...
DopplerShift_array     = fltarr(n_elements(Eclipse_files))
window, 0, xs = 800, ys = 1000
cgplot, WL, spec, xr = [7769.,7778.], yr = [5.e3, 4.e4], ytitle = 'R/A', /nodata
;cgplot, WL, spec, xr = [6556.,6568.], yr = [5.e3, 4.e4], ytitle = 'R/A', /nodata
for i = 1, n_elements(Eclipse_files)-1 do begin
  spec = MRDFITS(reduced_dir+'R_per_A_'+Eclipse_files[i]+'.ec.fits', 0, header, /fscale, /unsigned, /silent)
  spec_err = MRDFITS(reduced_dir+'sig_R_per_A_'+Eclipse_files[i]+'.ec.fits', 0, sig_header, /fscale, /unsigned, /silent)
  ...
  window, 2, xs = 800, ys = 1000
  res_coadd = findgen(n_elements(residual_array[*, 0]))/10000000. ;hack to make zero array sinceI am unsureof how to make one.
  wl_coadd = findgen(n_elements(wl_array[*, 0]))/10000000.
  cgplot, WL, spec, xr = [7769.,7778.], yr = [-1.e3, 7.e3], ytitle = 'Residual Jovian scattered light subtraction R/A', /nodata
  for i = 0, n_elements(Eclipse_files)-1 do begin
    cgplot, WL_array[*, i], residual_array[*, i], /OVERPLOT, COLOR = timeColors[i]
    res_coadd = res_coadd + residual_array[*, i]
    wl_coadd = wl_coadd + WL_array[*, i]
  endfor
  cgplot, wl_coadd/6, res_coadd, /OVERPLOT, COLOR = 'red'
stop
  ;---------------------A look at O I 8446----------------------------------
  parinfo[0].limits      = [0., 1.e6]
  Gaussian_parinfo       = replicate({value:0., fixed:0, limited:[0,0], limits:[0.,0.]}, 3)

  include_WLs            = where( ((wl gt 8440.) and (wl lt 8452.)), /NULL)
  ...
  DopplerShift_array     = fltarr(n_elements(Eclipse_files))
  window, 0, xs = 800, ys = 1000
  cgplot, WL, spec, xr = [8441.,8451.], yr = [5.e3, 4.e4], ytitle = 'R/A', /nodata
  ;cgplot, WL, spec, xr = [6556.,6568.], yr = [5.e3, 4.e4], ytitle = 'R/A', /nodata
  for i = 1, n_elements(Eclipse_files)-1 do begin
    spec = MRDFITS(reduced_dir+'R_per_A_'+Eclipse_files[i]+'.ec.fits', 0, header, /fscale, /unsigned, /silent)
    spec_err = MRDFITS(reduced_dir+'sig_R_per_A_'+Eclipse_files[i]+'.ec.fits', 0, sig_header, /fscale, /unsigned, /silent)
  ...
  res_coadd = findgen(n_elements(residual_array[*, 0]))/10000000. ;hack to make zero array sinceI am unsureof how to make one.
  wl_coadd = findgen(n_elements(wl_array[*, 0]))/10000000.
  cgplot, WL, spec, xr = [8441.,8451.], yr = [-1.e3, 7.e3], ytitle = 'Residual Jovian scattered light subtraction R/A', /nodata
  for i = 0, n_elements(Eclipse_files)-1 do begin
    cgplot, WL_array[*, i], residual_array[*, i], /OVERPLOT, COLOR = timeColors[i]
    res_coadd = res_coadd + residual_array[*, i]
    wl_coadd = wl_coadd + WL_array[*, i]
...
include_WLs            = where( ((wl gt 9229.) and (wl lt 9241.)), /NULL)
...
DopplerShift_array     = fltarr(n_elements(Eclipse_files))
window, 0, xs = 800, ys = 1000
cgplot, WL, spec, xr = [9230.,9240.], yr = [5.e3, 4.e4], ytitle = 'R/A', /nodata
;cgplot, WL, spec, xr = [6556.,6568.], yr = [5.e3, 4.e4], ytitle = 'R/A', /nodata
for i = 1, n_elements(Eclipse_files)-1 do begin
  spec = MRDFITS(reduced_dir+'R_per_A_'+Eclipse_files[i]+'.ec.fits', 0, header, /fscale, /unsigned, /silent)
  spec_err = MRDFITS(reduced_dir+'sig_R_per_A_'+Eclipse_files[i]+'.ec.fits', 0, sig_header, /fscale, /unsigned, /silent)
...
res_coadd = findgen(n_elements(residual_array[*, 0]))/10000000. ;hack to make zero array sinceI am unsureof how to make one.
wl_coadd = findgen(n_elements(wl_array[*, 0]))/10000000.
cgplot, WL, spec, xr = [9230.,9240.], yr = [-1.e3, 7.e3], ytitle = 'Residual Jovian scattered light subtraction R/A', /nodata
for i = 0, n_elements(Eclipse_files)-1 do begin
  cgplot, WL_array[*, i], residual_array[*, i], /OVERPLOT, COLOR = timeColors[i]
  res_coadd = res_coadd + residual_array[*, i]
  wl_coadd = wl_coadd + WL_array[*, i]
